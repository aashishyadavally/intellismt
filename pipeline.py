...
def explore_and_validate(all_constraints, all_smt2_constraints, placeholder, logs, args):
    ...
    all_candidates = llm_explorer.explore_with_self_consistency(
        explorer_prompt, seed, all_constraints
    )
    ...
    for candidate_id, (response, subset, usage) in enumerate(all_candidates):
        ...
        if subset:
            ...
            if is_unsat:
                validator_response = "UNSAT"
                if len(subset) < len(best_subset):
                    best_subset = deepcopy(subset)
                    best_subset_id = deepcopy(candidate_id)
            else:
                validator_response = "SAT"
    ...
    if best_subset_id:
        ...
        logs['Best candidate'] = None

    # If using ``embeddings`` for parsing, remove temporary cache for Explorer.
